...
import * as L from 'leaflet'; // Importa Leaflet con tipado
    ...
    if (isClient) {
      ...
      if (mapElement && !mapElement._leaflet_id) {
        ...
        locations.forEach((location) => {
          const popupContent = `
            <div class="p-4">
              <h3 className="text-lg font-semibold text-green-700">${location.name}</h3>
              <p className="text-sm text-gray-700">Dirección: ${location.address}</p>
              <p className="text-sm text-gray-700">Horario: ${location.hours}</p>
            </div>
          `;
